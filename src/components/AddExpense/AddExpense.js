...
import './AddExpense.css'; // Импортируем стили
...
const AddExpense = ({ onClose }) => {
    ...
    const handleAddExpense = async () => {
        try {
            ...
            const response = await axios.post(
                'http://localhost:5555/budget/addExpense',
    ...
    return (
        <div className="add-expense-container">
            <h3 className="add-expense-title">Добавить растрату</h3>
            <input
                type="number"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
                placeholder="Сумма растраты"
            ...
            <input
                ...
                required
            />
            <button onClick={handleAddExpense} className="button">Добавить</button>
